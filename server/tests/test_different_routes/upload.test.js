// tests/test_different_routes/upload.test.js
const request = require('supertest');
const mongoose = require('mongoose');
const path = require('path');
const fs = require('fs');
const { app } = require("../../app");

jest.setTimeout(30000);

describe('文件上传模块路由测试 (Upload Routes)', () => {

  // === 环境准备 ===
  beforeAll(async () => {
    const TEST_URI = process.env.MONGODB_URI_TEST || 'mongodb://127.0.0.1:27017/yisu_test_upload';
    if (mongoose.connection.readyState === 0) {
      await mongoose.connect(TEST_URI);
    }

    // 确保上传目录存在
    const uploadDir = path.join(__dirname, '../../public/uploads');
    if (!fs.existsSync(uploadDir)) {
      fs.mkdirSync(uploadDir, { recursive: true });
    }
  });

  afterAll(async () => {
    await mongoose.connection.close();
  });

  // ==========================================
  // 1. 上传图片 (POST /api/upload)
  // ==========================================
  describe('POST /api/upload (上传图片)', () => {

    // 创建临时测试图片
    const testImagePath = path.join(__dirname, 'test-image.jpg');
    const testTextPath = path.join(__dirname, 'test-file.txt');

    beforeEach(() => {
      // 创建一个简单的测试图片 (1x1 红色像素的 JPEG)
      // 这是一个最小的有效 JPEG 文件
      const minimalJpeg = Buffer.from([
        
      ]);
      fs.writeFileSync(testImagePath, minimalJpeg);

      // 创建一个文本文件 (非图片)
      fs.writeFileSync(testTextPath, 'This is not an image');
    });

    afterEach(() => {
      // 清理测试文件
      if (fs.existsSync(testImagePath)) {
        fs.unlinkSync(testImagePath);
      }
      if (fs.existsSync(testTextPath)) {
        fs.unlinkSync(testTextPath);
      }
    });

    it('1.1 上传成功：返回图片 URL', async () => {
      const res = await request(app)
        .post('/api/upload')
        .attach('file', testImagePath);

      expect(res.statusCode).toBe(200);
      expect(res.body.msg).toBe('上传成功');
      expect(res.body.url).toMatch(/^\/uploads\//);
    });

    it('1.2 无文件：返回 400', async () => {
      const res = await request(app)
        .post('/api/upload');

      expect(res.statusCode).toBe(400);
      expect(res.body.msg).toBe('请选择要上传的文件');
    });

    it('1.3 非图片文件：应被拒绝', async () => {
      const res = await request(app)
        .post('/api/upload')
        .attach('file', testTextPath);

      // 非图片文件应被拒绝，返回 400
      expect(res.statusCode).toBe(400);
      expect(res.body.msg).toBe('只允许上传图片文件');
    });
  });
});
